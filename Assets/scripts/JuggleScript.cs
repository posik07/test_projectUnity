...
public class JuggleScript : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (positionJiggler)
            ...
            basePosition + positionJigglerAmount * Mathf.Sin(positionTime) * power;

        if (rotationJiggler)
            transform.localRotation =
            baseRotation * Quaternion.Euler(
            rotationJigglerAmount * Mathf.Sin(rotationTime) * power);

        if (scaleJiggler)
            transform.localScale =
            baseScale + scaleJigglerAmount * Mathf.Sin(scaleTime) * power;
    }
